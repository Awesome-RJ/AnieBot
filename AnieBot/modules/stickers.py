...
@Abot(pattern="^/kang(@MissAnieBot_Bot)? ?(.*)")
async def kang(event):
    ...
    if msg.sticker:
        ...
        await sended.delete()
    short_name = f"nk{event.sender_id}_by_MissAnieBot_Bot"
    user_id = OWNER_ID
...
@Abot(pattern="^/unkang(@MissAnieBot_Bot)?$")
...
@Abot(pattern="^/mypac(k|ks)(@MissAnieBot_Bot)? ?(.*)")
async def my_pack(e):
...
@Abot(pattern="^/(pkang|packkang)(@MissAnieBot_Bot)? ?(.*)")
...
@Abot(pattern="^/stickers(@MissAnieBot_Bot)? ?(.*)")
