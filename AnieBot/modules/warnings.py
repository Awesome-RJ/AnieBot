...
@Abot(pattern="^/resetallwarns(@DecodeManagerBot)?$")
...
@Abot(pattern="^/(warn|swarn|dwarn)(@DecodeManagerBot|DecodeManagerBot)? ?(.*)")
async def warn_peepls____(e):
    ...
    if not warn:
        ...
        if strength == "tban":
            ...
            action = f"banned for{get_time(actiontime)}"
        elif strength == "tmute":
            await tbot.edit_permissions(
                e.chat_id,
                user.id,
            ...
            action = f"muted for{get_time(actiontime)}"
        elif strength == "ban":
...
@Abot(pattern="^/warns(@DeCodeManagerBot)? ?(.*)")
...
@Abot(pattern="^/rmwarn(@DeCodeManagerBot)? ?(.*)")
