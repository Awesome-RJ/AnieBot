...
@Abot(pattern="^/(webss|sshot|screenshot) ?(.*)")
async def _(event):
    try:
...
async def up(event):
    ...
    if vote_up:
        ...
        add_vote_up(event_id, event.sender_id)
        count1 += 1
    cb_data = f'{event_id}|{count1}|{count2}'
    C1 = count1
    C2 = count2
    if C1 == 0:
        C1 = ""
...
@Abot(pattern="^/(stat|stat)(@MissAnieBot_Bot|@missAnieBotbot)?$")
...
telegraph = Telegraph()
telegraph.create_account(short_name="AnieBot")


@Abot(pattern="^/telegraph(@MissAnieBot_Bot)? ?(.*)")
