...
import AnieBot.modules.mongodb.sudos_db as sdb
import AnieBot.modules.sql.elevated_users_sql as sql
...
@Abot(pattern="^/ping(@DeCodeManagerBot)?$")
async def ping(event):
    ...
    final = f'{str(final.microseconds)[:3]} ms'
    text = "<b>PONG!!</b>"
...
@Abot(pattern="^/feedback(@DeCodeManagerBot)? ?(.*)")
...
async def sudo_list(event):
    if (
        ...
        return await event.reply(
            "You don't have access to use this, visit @AnieBotChan_Support."
        )
...
@Abot(pattern="^/broadcast ?(.*)")
async def bc(event):
    if event.sender_id not in [OWNER_ID, 5046520072]:
        return await event.reply(
...
@Abot(pattern="^/update$", from_users=[OWNER_ID])
async def updator__(e):
    result = subprocess.run(["git", "pull"], stdout=subprocess.PIPE)
    rp = f"`AnieBot:~$ git pull\n {result.stdout.decode()}`"
    await e.reply(rp)
    if rp == "Already up to date.":
        return
    args = [sys.executable, "-m", "AnieBot"]
    os.execle(sys.executable, *args, os.environ)


@Abot(pattern="^/restart$", from_users=[OWNER_ID])
async def restart(e):
