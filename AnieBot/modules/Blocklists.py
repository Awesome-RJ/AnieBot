...
import AnieBot.modules.sql.blacklist_sql as sql
import AnieBot.modules.sql.warns_sql as wsql
...
@Abot(pattern="^/(blocklist|blacklist)$")
async def _(event):
    if event.is_private:
        return  # connect
...
@Abot(pattern="^/(rmblacklist|rmblocklist) ?(.*)")
async def _(event):
    if event.is_private:
        return  # connect
    if not await can_change_info(event, event.sender_id):
...
@Abot(pattern="^/(unblocklistall|unblacklistall)$")
async def _(event):
    if event.is_private:
        return  # connect
...
@Abot(pattern="^/(blocklistmode|blacklistmode) ?(.*)")
async def _(event):
    if event.is_private:
        return  # connect
    if not await can_change_info(event, event.sender_id):
