...
async def text_captcha(event, chat_id):
    ...
    ans = [sol]
    ans.extend(iter(wrong))
    shuffle(ans)
    btns = []
    bt = []
    for x in ans:
        cb_data = f'{str(chat_id)}|{str(x)}|{str(sol)}'
...
async def txtc(event):
    ...
    if option != ans:
        ...
        bt = []
        for x in ans:
            cb_data = f'{chat_id}|{str(x)}|{str(character)}'
            bt.append(Button.inline(x, data=f"txtc_{cb_data}"))
            if len(bt) == 3:
                btns.append(bt)
                bt = []
        chance = check.find_one({"chat_id": chat_id, "user_id": event.sender_id})
        chance = int(chance["chance"]) - 1 if chance else 3
        if chance == 0:
            check.update_one(
                {"chat_id": chat_id, "user_id": event.sender_id},
                {"$set": {"chance": 3}},
...
async def math_captcha(event, chat_id):
    ...
    ans = [solution]
    ans.extend(randint(0, 999) for _ in range(8))
    shuffle(ans)
    btns = []
    bt = []
    for x in ans:
        cb_data = f'{str(chat_id)}|{str(x)}|{str(solution)}'
