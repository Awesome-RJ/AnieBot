...
async def cb(e):
    ...
    for x in [".", "!", "/", "?", "+"]:
        if q.startswith(x):
            return
    if "AnieBot" in q.lower():
        q = (q.lower()).replace("AnieBot", "kuki")
    async with tbot.action(e.chat_id, "typing"):
        async with ubot.conversation("@KukiAI_bot") as chat:
            ...
            for x in ["Kuki", "kuki.", "Kuki.", "kuki"]:
                response = response.replace(x, "AnieBot")
            if "Fams" in response:
...
The **AI** replies to anyone who tags the bot or mentions "AnieBot" in their text.
"""
