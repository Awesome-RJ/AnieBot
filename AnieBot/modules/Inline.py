...
from .. import tbot
from ..utils import Sammy, Cquery
...
async def gen_help(event, thumb):
    ...
    return await event.builder.article(
        ...
        description="Inline query help menu of AnieBot chan.",
...
async def whisper_message__(event):
    ...
    if query == "" or len(query) > 4096:
        content = "**Send whisper messages through inline mode**\n\nUsage: `@MissAnieBot_Bot [@username] text`"
        des = "Usage: @MissAnieBot_Bot [@username] text"
...
@Sammy(pattern=r"show_whisper(\_(.*))")
...
async def doge(event):
    ...
    if not N:
        return
    image = Image.open("AnieBot/modules/sql/IMG_20210705_134908_649.jpg")
    font = ImageFont.truetype(
        "AnieBot/modules/sql/FontsFree-Net-Ambiguity-radical.ttf", 70
    )
