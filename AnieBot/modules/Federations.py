...
@Abot(pattern="^/newfed(@Aniebot_robot)? ?(.*)")
...
@Sammy(pattern=r"cancel_delete")
async def delete_fed(event):
    await event.edit("Federation deletion cancelled.")


@Abot(pattern="^/renamefed ?(.*)")
...
@Abot(pattern="^/fpromote(@Aniebot_robot)? ?(.*)")
async def fp(event):
    if event.is_private:
        return await event.reply(
    ...
    cb_data = f'{str(event.sender_id)}|{str(user.id)}'
...
@Abot(pattern="^/(ftransfer|fedtransfer)(@Aniebot_robot)? ?(.*)")
async def ft(event):
    if event.is_private:
        return await event.reply(
            "This command is made to be used in group chats, not in pm!"
    ...
    cb_data = f'{str(sender_id)}|{str(user_r.id)}'
...
async def ft(event):
    ...
    cb_data = f'{str(owner.id)}|{user_id}'
    buttons = [
...
async def fban(event):
    ...
    if fban:
        ...
        if reason:
            fban_global_text = f"{fban_global_text}<b>Reason:</b> {reason}"
...
@Sammy(pattern=r"check_fadmins(\_(.*))")
...
@Abot(pattern="^/(feddemoteme|fdemoteme) ?(.*)")
...
@Abot(pattern="^/(fexport|fedexport)(@Aniebot_robot)? ?(.*)")
...
@Abot(pattern="^/(fimport|fedimport)(@Aniebot_robot)? ?(.*)")
