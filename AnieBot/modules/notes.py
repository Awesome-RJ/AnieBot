...
import AnieBot.modules.mongodb.notes_db as db
from AnieBot import tbot
from AnieBot.utils import Abot, Sammy
...
async def pnotes(event):
    ...
    if not arg:
        ...
        if mode:
            await event.reply(
                "Your notes are currently being sent in private. AnieBot will send a small note with a button which redirects to a private chat."
            )
        else:
            await event.reply("Your notes are currently being sent in the group.")
    elif arg in ["y", "yes", "on"]:
        await event.reply(
            "AnieBot will now send a message to your chat with a button redirecting to PM, where the user will receive the note."
        )
        db.change_pnotes(event.chat_id, True)
    elif arg in ["n", "no", "off"]:
        await event.reply("AnieBot will now send notes straight to the group.")
...
async def new_message_note(event):
    ...
    if p_mode == False:
        ...
        await event.respond(
            ...
            buttons=Button.url(
                "Click me",
                data=f"t.me/MissAnieBot_bot?start=notes_{event.chat_id}|{name}",
            ),
            reply_to=event.reply_to_msg_id or event.id,
        )
...
@Abot(pattern="^/(saved|Saved|Notes|notes)")
async def alln(event):
    ...
    if p_mode:
        await event.respond(
            ...
            buttons=Button.url(
                "Click Me!",
                f"t.me/MissAnieBot_bot?start=allnotes_{event.chat_id}",
            ),
            reply_to=event.reply_to_msg_id or event.id,
        )
    else:
...
async def rr(event):
    ...
    for a_note in all_notes:
        ...
        OUT_STR += f"- [{a_note}](t.me/MissAnieBot_bot?start=notes_{luv})\n"
