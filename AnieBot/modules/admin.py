...
@Abot(pattern="^/(superpromote|fullpromote)(?: |$|@Aniebot_robot)(.*)")
...
@Abot(pattern="^/demote(?: |$|@Aniebot_robot)(.*)")
async def _de(e):
    if e.is_private:
        return await e.reply(
            "This command is made to be used in group chats, not in my PM!"
...
@Abot(pattern="^/setgtitle(?: |$|@Aniebot_robot)(.*)")
async def x_title(e):
    if not e.is_group:
        return await e.reply("This command is made to be used in groups!")
    if e.from_id:
