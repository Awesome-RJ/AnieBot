...
from AnieBot import BOT_ID, OWNER_ID, tbot, ubot
...
@Abot(pattern="^/purge(?: |$|@MissAnieBot_Bot)(.*)")
...
@Abot(pattern="^/del(?: |$|@MissAnieBot_Bot)(.*)")
...
@Abot(pattern="^/spurge(?: |$|@MissAnieBot_Bot)(.*)")
async def b(event):
    lt = event.pattern_match.group(1)
    if lt and not lt.isdigit():
        lt = None
