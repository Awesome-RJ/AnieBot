...
from AnieBot import tbot, ubot
from AnieBot.modules.sql.nightmode_sql import (
    add_nightmode,
    get_all_chat_id,
    is_nightmode_indb,
    rmnightmode,
)
from AnieBot.utils import Abot
...
@Abot(pattern="^/(jackpot|dice|dart|goal|football|basketball|bowling)$")
...
@Abot(pattern="^/(crypto|btc|Crypto|BTC|Btc|ETH|Eth|eth|DOGE|Doge|doge|ltc)$")
async def kek(event):
...
@Abot(pattern="^/(yts|movie|Yts|Movie) ?(.*)")
async def _(event):
