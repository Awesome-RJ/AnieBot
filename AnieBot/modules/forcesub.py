...
from AnieBot import BOT_ID, CMD_HELP, OWNER_ID, tbot
...
@Abot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
async def fsub_n(e):
    ...
    if (
        e.chat.admin_rights
        and not e.chat.admin_rights.ban_users
        or not e.chat.admin_rights
    ):
        return
    if not e.from_id:
        return
