...
def set_welcome_mode(chat_id: str, mode):
    wel = SESSION.query(Wlc).get(chat_id)
    if wel:
        wel.mode = mode
    else:
...
def welcome_mode(chat_id: str):
    return wel.mode if (wel := SESSION.query(Wlc).get(chat_id)) else True
...
def set_goodbye_mode(chat_id: str, mode):
    goodb = SESSION.query(GB).get(chat_id)
    if goodb:
        goodb.mode = mode
    else:
...
def goodbye_mode(chat_id: str):
    return goodb.mode if (goodb := SESSION.query(GB).get(chat_id)) else True
