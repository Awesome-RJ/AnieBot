...
import AnieBot.modules.mongodb.welcome_db as db
import AnieBot.modules.sql.captcha_sql as sql
...
"""
        if chat_s:
            if chat_s["text"] or chat_s["id"]:
                ...
                s_mode = chat_s["mode"] if chat_s and chat_s["mode"] else True
...
Promote me as administrator in your group otherwise I will not function properly""",
            buttons=[
                [Button.url("Support Chat", "https://t.me/AnieBotChan_Support")],
                [Button.url("Updates Channel", "https://t.me/AnieBotChan_Updates")],
            ],
        )
    ...
    if sql.get_mode(chat_id) == True:
        ...
        if style in ["math", "text"]:
            custom_welcome = (
                custom_welcome
                + f"[Click here to prove human](btnurl://t.me/MissAnieBot_Bot?start=captcha_{chat_id})"
            )
    if custom_welcome:
